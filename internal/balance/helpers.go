...
// blockTrackingEmitter wraps an Emitter and reports how long the current or last
// stdout write has been blocked by downstream backpressure. Because the
// underlying write blocks when the OS pipe fills, a write that never returns (a
// genuinely wedged sink) would never update the gauge if measured only on
...
// Emit publishes the growing blocked duration of the underlying write.
...
// noopMetrics satisfies Metrics with no-ops, so a Poller can run without a
...
func (noopMetrics) SetLastSampledBlock(uint64)                    {}
func (noopMetrics) SetLagBlocks(uint64)                           {}
func (noopMetrics) SetEmitBlockedSeconds(float64)                 {}
func (noopMetrics) IncSampleRecord()                              {}
func (noopMetrics) IncChangeRecord()                              {}
func (noopMetrics) SetAccountBalanceWei(string, string, *big.Int) {}
func (noopMetrics) SetAccountBalanceEth(string, string, float64)  {}
func (noopMetrics) SetAccountTokenBalanceRaw(string, string, string, string, *big.Int) {
}
func (noopMetrics) SetAccountTokenBalance(string, string, string, string, float64) {}
